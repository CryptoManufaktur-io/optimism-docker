...
# Trusted/static nodes: if EXECUTION_P2P_TRUSTED_NODES defined (JSON array), attempt to produce a static-nodes file
if [ -n "${EXECUTION_P2P_TRUSTED_NODES:-}" ]; then
  echo "Writing trusted/static nodes to /var/lib/op-reth/static-nodes.json"
  # Expect EXECUTION_P2P_TRUSTED_NODES as JSON array like ["enode://...","enode://..."]
  echo "${EXECUTION_P2P_TRUSTED_NODES}" | jq -c '.' > /var/lib/op-reth/static-nodes.json || true
fi
